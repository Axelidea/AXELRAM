...
\caption{Processing flow comparison. Conventional (top): GPU computes K,V, quantizes in software, stores to HBM, reads back with software dequantization---$d \times T$ multiplications per query. AXELRAM (bottom): single-step write (Hadamard + quantize + SRAM store) and single-step read (table lookup + add)---1 multiplication per key, on-chip SRAM only.}
...
Instead of inverse-transforming each stored key ($T$ times), we transform the query once and compute in the rotated space:
\begin{equation}
...
The read path (Fig.~\ref{fig:macro}, right) operates as follows.
Upon query arrival:
\begin{enumerate}[leftmargin=*,nosep]
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_readpath.png}
\caption{Read path detail. Phase~1 (once per query): FWHT rotation of query, table generation ($d \times 2^b$ multiplications). Phase~2 (repeated $T$ times): table lookup, adder tree, norm scaling (1 multiplication per key).}
\label{fig:readpath}
\end{figure}
